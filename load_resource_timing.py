...
def autopsy_http(packet, objects, tcpQueue):
    p = packet # for short
    if 'tcp' not in p:
    ...
    if 'request_method' in p.http.field_names:
        ...
        # hopefully this is the last chunk
        objects[tcpStream][streamId]['data'].append(now)


    return

def autopsy_http_tcp(packet, objects, tcpQueue):
    p = packet # for short
...
def retrive_timing(dump):
    ...
    for p in dump: # for all packets
        if 'http2' in p:
            ...
            # HTTP
            autopsy_http(p, objects, tcpQueue)
        elif 'ssl' in p:
            ...
            if 'stream' in p.tcp.field_names and int(p.tcp.stream) in tcpQueue:
                ...
                autopsy_http_tcp(p, objects, tcpQueue)
        else:
            # other protocols
            continue
...
def main():
    cap = pyshark.FileCapture('/Users/ywu/cwpf_test/h2.pcap', sslkey_path='/Users/ywu/cwpf_test/ssl_keylog', http_only=True)
